...
function isTodayInSelectedDays(selectedDays: string[]): boolean {
  if (!selectedDays || selectedDays.length === 0) {
    return true; // If no days specified, allow all days
  }
...
// Check if notifications should run based on settings
export function shouldScheduleNotifications(
  ...
  if (!isTodayInSelectedDays(selectedDays)) {
    console.log('📅 Today is not in selected notification days');
    return false;
  }

  // Optionally check time window (not strictly required for scheduling)
  if (startTime && endTime && !isWithinNotificationWindow(startTime, endTime)) {
    console.log('⏰ Outside notification time window');
    // Still return true - we'll schedule for the right times
  }
...
// UPDATED: Schedule language learning reminders with custom content, navigation data, and day selection
export async function scheduleLanguageLearningReminders(
  startTime: string = "09:00",
  endTime: string = "18:00", 
  frequency: number = 3,
  ...
  try {
    ...
    console.log(`📅 Scheduling notifications for days: ${selectedDays.join(', ')}`);
    ...
    for (const dayName of selectedDays) {
      ...
      // Schedule multiple notifications throughout each selected day
      for (let i = 0; i < frequency; i++) {
        const notificationTime = startMinutes + (interval * i) + Math.random() * 15; // Add slight randomness
        const hour = Math.floor(notificationTime / 60);
        const minute = Math.floor(notificationTime % 60);
    ...
    const totalNotifications = selectedDays.length * frequency;
    console.log(`✅ Scheduled ${totalNotifications} notifications (${frequency} per day on ${selectedDays.length} days) for ${language} between ${startTime} and ${endTime}`);
    return true;
  } catch (error) {
    console.error('Error scheduling notifications:', error);
    return false;
