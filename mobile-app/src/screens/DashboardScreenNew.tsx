...
import { scheduleLanguageLearningReminders, shouldScheduleNotifications } from '../lib/notifications';
...
useEffect(() => {
  ...
  // AUTO-SCHEDULE: Schedule notifications automatically based on mobile settings
  useEffect(() => {
    const autoScheduleNotifications = async () => {
      try {
        // Get mobile notification settings from dashboard data
        const settings = effectiveDashboardData?.settings;
        
        if (!settings) {
          console.log('⏭️ No settings available yet, skipping auto-schedule');
          return;
        }

        // Check if we should schedule based on mobile settings
        const shouldSchedule = shouldScheduleNotifications(
          settings.mobileNotificationsEnabled || false,
          settings.mobileNotificationDays || ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
          settings.mobileNotificationStartTime || '09:00',
          settings.mobileNotificationEndTime || '18:00'
        );

        if (!shouldSchedule) {
          console.log('📵 Auto-schedule skipped - notifications disabled or not scheduled for today');
          return;
        }
        ...
        if (status !== 'granted') {
          console.log('🔕 Notification permission not granted, skipping auto-schedule');
          return;
        }
        ...
        if (existingNotifications.length > 0) {
          console.log(`✅ ${existingNotifications.length} notifications already scheduled, skipping auto-schedule`);
          return;
        }

        // Get next lesson for notification data
        const nextLesson = upcomingLessons?.[0];
        
        // Auto-schedule notifications
        console.log('🚀 Auto-scheduling notifications based on mobile settings');
        await scheduleLanguageLearningReminders(
          settings.mobileNotificationStartTime || '09:00',
          settings.mobileNotificationEndTime || '18:00',
          settings.mobileNotificationFrequency || 3,
          settings.language || user?.selectedLanguage || 'italian',
          nextLesson ? { courseId: nextLesson.courseId, lessonId: nextLesson.lessonId } : undefined,
          settings.mobileNotificationDays || ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
        );

        console.log('✅ Notifications auto-scheduled successfully');
      } catch (error) {
        console.error('❌ Error auto-scheduling notifications:', error);
      }
    };

    // Only run auto-schedule when we have dashboard data
    if (effectiveDashboardData?.settings) {
      autoScheduleNotifications();
    }
  }, [
    effectiveDashboardData?.settings?.mobileNotificationsEnabled,
    effectiveDashboardData?.settings?.mobileNotificationDays,
    effectiveDashboardData?.settings?.language,
  ...
  const handleMenuPress = () => {
    (navigation.navigate as any)('MainTabs', { screen: 'Profile' });
  };
