...
export default function LessonScreen() {
  ...
  // Track audio configuration per video instance
  const audioModeConfiguredRef = useRef<Record<string, boolean>>({});
  ...
  const getCurrentStepData = () => {
    try {
      ...
      if (currentLesson.lesson?.steps && !Array.isArray(currentLesson.lesson.steps)) {
        // Determine step 4 name (could be 'pro_video' or 'video_choice')
        const step4Name = 'pro_video' in currentLesson.lesson.steps ? 'pro_video' : 
                         'video_choice' in currentLesson.lesson.steps ? 'video_choice' : 'pro_video';
  ...
  // Configure audio to play in silent mode (iOS) - per video instance
  const createVideoPlaybackHandler = (videoId: string) => async (status: any) => {
    if (status.isLoaded && status.isPlaying && !audioModeConfiguredRef.current[videoId]) {
      try {
        ...
        audioModeConfiguredRef.current[videoId] = true;
        console.log(`✅ Audio configured to play in silent mode for video: ${videoId}`);
      } catch (error) {
        console.error(`❌ Failed to configure audio mode for video ${videoId}:`, error);
      }
    }
    
    // Reset the flag when playback stops so this video can configure audio again
    if (status.isLoaded && !status.isPlaying && audioModeConfiguredRef.current[videoId]) {
      audioModeConfiguredRef.current[videoId] = false;
    }
  };
            ...
            <CardContent style={styles.introVideoContent}>
              ...
              <View style={styles.introVideoContainer}>
                <Video
                  ...
                  shouldPlay={true}
                  isMuted={false}
                  onPlaybackStatusUpdate={createVideoPlaybackHandler('intro-video')}
                />
              </View>
          ...
          <Card style={styles.lessonCard}>
            <CardContent style={styles.lessonContent}>
                  ...
                  <View style={styles.videoContainer}>
                    <Video
                      ...
                      source={(() => {
                        ...
                        // Static mapping for local IRL video files
                        // Course 1 IRL videos
                        if (url.includes('street_or_cafe_greeting') || url.includes('cafe_greeting')) 
                          return require('../attached_assets/videos/italian/beginners/street_or_cafe_greeting.mp4');
                        if (url.includes('cafe_farewell')) 
                          return require('../attached_assets/videos/italian/beginners/cafe_farewell.mp4');
                        if (url.includes('doorway_goodbye')) 
                          return require('../attached_assets/videos/italian/beginners/doorway_goodbye.mp4');
                        
                        // Course 2 IRL videos
                        if (url.includes('party_introduction')) 
                          return require('../attached_assets/videos/italian_beginners_course2/party_introduction.mp4');
                        if (url.includes('where_are_you_from')) 
                          return require('../attached_assets/videos/italian_beginners_course2/where_are_you_from.mp4');
                        if (url.includes('first_meeting_farewell')) 
                          return require('../attached_assets/videos/italian_beginners_course2/first_meeting_farewell.mp4');
                        
                        // Course 3 IRL videos
                        if (url.includes('cafe_thank_you')) 
                          return require('../attached_assets/videos/italian_beginners_course3/cafe_thank_you.mp4');
                        if (url.includes('get_attention_formal')) 
                          return require('../attached_assets/videos/italian_beginners_course3/get_attention_formal.mp4');
                        if (url.includes('apology_response')) 
                          return require('../attached_assets/videos/italian_beginners_course3/apology_response.mp4');
                      ...
                      resizeMode={ResizeMode.CONTAIN}
                      shouldPlay={false}
                      onPlaybackStatusUpdate={createVideoPlaybackHandler('irl-video')}
                    />
                  </View>
                ...
                <>
                  ...
                  <View style={styles.videoContainer}>
                    <Video
                      ...
                      source={(() => {
                        const url = stepData.videoUrl;
                        // Static mapping for local video files
                ...
                <>
                  ...
                  {stepData.hasAccess ? (
                      ...
                      <View style={styles.videoContainer}>
                        <Video
                          ...
                          source={(() => {
                            ...
                            // Gender-based videos (lesson 1)
                            if (url.includes('lesson1_hi_neutral')) return require('../attached_assets/videos/lesson1_hi_neutral.mp4');
                            if (url.includes('lesson1_hi_female')) return require('../attached_assets/videos/lesson1_hi_female.mp4');
                            if (url.includes('lesson1_hi_male')) return require('../attached_assets/videos/lesson1_hi_male.mp4');
                            
                            // Pro videos - precise matching (check double-digit first to avoid prefix collisions)
                            if (url.match(/lesson13[^0-9]/)) return require('../attached_assets/videos/lesson13_pro.MOV');
                            if (url.match(/lesson12[^0-9]/)) return require('../attached_assets/videos/lesson12_pro.MOV');
                            if (url.match(/lesson11[^0-9]/)) return require('../attached_assets/videos/lesson11_pro.MOV');
                            if (url.match(/lesson10[^0-9]/)) return require('../attached_assets/videos/lesson10_pro.MOV');
                            if (url.match(/lesson9[^0-9]/)) return require('../attached_assets/videos/lesson9_pro.MP4');
                            if (url.match(/lesson8[^0-9]/)) return require('../attached_assets/videos/lesson8_pro.MOV');
                            if (url.match(/lesson7[^0-9]/)) return require('../attached_assets/videos/lesson7_pro.MOV');
                            if (url.match(/lesson6[^0-9]/)) return require('../attached_assets/videos/lesson6_pro.MOV');
                            if (url.match(/lesson5[^0-9]/)) return require('../attached_assets/videos/lesson5.mp4');
                            if (url.match(/lesson4[^0-9]/)) return require('../attached_assets/videos/lesson4.mp4');
                            if (url.match(/lesson3[^0-9]/)) return require('../attached_assets/videos/lesson3.mp4');
                          ...
                          resizeMode={ResizeMode.CONTAIN}
                          shouldPlay={false}
                          onPlaybackStatusUpdate={createVideoPlaybackHandler('pro-video')}
                        />
                      </View>
                    ...
                    <>
                      ...
                      <TouchableOpacity 
                        onPress={async () => {
                          // Check if there's a step 5 (text_tip)
                          const hasStep5 = Array.isArray(currentLesson?.lesson?.steps) 
                            ? currentLesson.lesson.steps.some((step: any) => step.stepNumber === 5)
                            : (currentLesson?.lesson?.steps && 'text_tip' in currentLesson.lesson.steps);
                          
                          if (hasStep5) {
                            // Navigate to step 5 instead of completing
                            setCurrentStep(5);
                            setSelectedAnswer("");
                            setShowResult(false);
                            setIsCorrect(false);
              ...
              {stepData && stepData.type === 'text_tip' && (
                <>
                  <View style={[styles.wordReviewContainer, { backgroundColor: '#EFF6FF' }]}>
                    <Text style={styles.lessonEmoji}>💡</Text>
                    <Text style={[styles.stepTitle, { color: '#1e40af', marginBottom: 16 }]}>
                    ...
                    <Text style={[styles.translationText, { color: '#1e3a8a', fontSize: 16, textAlign: 'center' }]}>
                      {(() => {
                  ...
                  <View style={{ marginTop: 16, alignItems: 'center' }}>
                    <Text style={[styles.noteText, { marginBottom: 16, color: '#6B7280' }]}>
                ...
                <>
                  ...
                  <View style={styles.inputContainer}>
                    <TextInput
                      style={[
                        styles.textInput,
                        showResult && !isCorrect && styles.incorrectTextInput
                      ]}
                      value={selectedAnswer}
                      onChangeText={setSelectedAnswer}
              ...
              {stepData && (
                <View style={styles.quizSection}>
                  {showResult && !isCorrect && stepData.type !== 'word_review' && (
                  ...
                  {!showResult ? (
                    ...
                    <Button
                      title={(() => {
                        if (stepData.isReview) return "Next";
                        // Check if there's a step 5 (text_tip)
                        const hasStep5 = Array.isArray(currentLesson?.lesson?.steps) 
                          ? currentLesson.lesson.steps.some((step: any) => step.stepNumber === 5)
                          : (currentLesson?.lesson?.steps && 'text_tip' in currentLesson.lesson.steps);
                        const maxStep = hasStep5 ? 5 : 4;
                        return currentStep < maxStep ? "Next" : "Complete Lesson";
                      })()}
                      onPress={handleNextStep}
