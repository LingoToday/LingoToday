...
import { Globe, Bell, BookOpen, TrendingUp, Sparkles, Zap, Users, Clock, Brain, Target, CheckCircle, Smartphone, Video, MessageSquare, Crown, ChevronDown } from "lucide-react";
...
export default function Landing() {
  const handleGoogleAuth = async () => {
    try {
      ...
      if (response.status === 503) {
        alert('Google authentication is not currently available. Please try again later.');
        return;
      }
      ...
      alert('Unable to connect to Google authentication. Please try again later.');
    }
  };
  ...
  const handleGitHubAuth = async () => {
    try {
      ...
      if (response.status === 503) {
        alert('GitHub authentication is not currently available. Please try again later.');
        return;
      }
      ...
      alert('Unable to connect to GitHub authentication. Please try again later.');
    }
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  <DropdownMenuItem onClick={handleGoogleAuth} className="cursor-pointer">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button 
                  size="lg" 
                  variant="outline"
                  className="border-gray-300 text-gray-700 font-medium px-8 py-3 rounded-full hover:bg-gray-50 flex items-center gap-2"
                >
                  Sign In
                  <ChevronDown className="w-4 h-4" />
                </Button>
